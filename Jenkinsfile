  ...
  stages {
    ...
    stage('Master-Deploy') {
      ...
      steps {
        ...
        sh 'sshpass -p $SCRIM_SEARCH_PASSWORD ssh -oStrictHostKeyChecking=no scrimsearch@$SERVER "(cd $SCRIM_SEARCH_LOCATION/ && NASCBOT_TOKEN=$NASCBOT_TOKEN INTERPROCESSAUTHKEY=$INTERPROCESSAUTHKEY SCRIM_FINDER_DB=$SCRIMFINDERDB POSTGRESHOST=$POSTGRESHOST POSTGRESPORT=$POSTGRESPORT SCRIMFINDERUSER=$SCRIMFINDERUSER SCRIMFINDERPASS=$SCRIMFINDERPASS SCRIM_FINDER_ENVIRONMENT=$ENVIRONMENT  docker-compose up --build -d)"'
      }
    }
  }
}